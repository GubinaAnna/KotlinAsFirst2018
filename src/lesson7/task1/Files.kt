...
fun sibilants(inputName: String, outputName: String) {
    val regex = """[жчшщ][ыяю]""".toRegex(RegexOption.IGNORE_CASE)
    File(outputName).writeText(
            File(inputName).readText().replace(regex) {
                val a = it.value[1]
                it.value[0].toString() + when (a.toLowerCase()) {
                    'ы' -> a - ('ы' - 'и')
                    'я' -> a - ('я' - 'а')
                    else -> a - ('ю' - 'у')
                }
...
fun chooseLongestChaoticWord(inputName: String, outputName: String) = {
    val differentLetterWords = File(inputName).readLines()
            .filter { it.length == it.toLowerCase().toList().distinct().size }
    if (differentLetterWords.isNotEmpty()) {
